...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchLeaderboardData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/leaderboards/get/${quizId}`); // Fetch leaderboard data for the quiz
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-4xl mx-auto">
        <nav className="mb-8">
          <ul className="flex space-x-4 justify-center">
            {[{ name: 'Home', path: '/' }, { name: 'Leaderboard', path: `/leaderboard/${quizId}` }, { name: 'Analytics', path: '/analytics' }, { name: 'Profile', path: '/profile' }].map(item => (
              <li key={item.name}>
                <Link to={item.path} className={`text-sm font-medium transition-colors hover:text-primary ${item.name === 'Leaderboard' ? 'text-primary' : 'text-muted-foreground'}`}>
                  {item.name}
                </Link>
